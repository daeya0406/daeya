...
function GuidePageContent() {
  ...
  const onChange = (key: TabKey) => {
    ...
    params.set('tab', key);
    router.replace(`/hobby/guide?${params.toString()}`, { scroll: false });
  };
  ...
  return (
    <section className="mx-auto max-w-5xl space-y-8">
      <header className="max-w-2xl">
        <h1 className="text-foreground text-3xl font-bold">디자인 시스템</h1>
        <p className="text-muted-foreground mt-3 leading-relaxed">
          이 사이트에 쓰는 UI·폰트·컬러 토큰을 정리한 디자인 시스템입니다.
        </p>
      </header>
