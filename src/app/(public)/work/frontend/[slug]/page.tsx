...
import { getFrontendProjectById, getFrontendProjects } from '@/content/registry';
...
const getProjectBySlug = (slug?: string | string[]) => {
  ...
  return (
    getFrontendProjectById(resolved) ??
    getFrontendProjectById(normalized) ??
...
export function generateStaticParams() {
  const base = getFrontendProjects().map((project) => ({ slug: project.id }));
...
export default async function FrontendCaseStudyPage({
  params,
}: {
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-10">
      ...
      >
        <Icon name="arrowLeft" size={16} /> Frontend
      </Link>
      ...
      {caseStudy.learnings && caseStudy.learnings.length > 0 && (
        <section className={['p-6 lg:p-8', cardClassName].join(' ')}>
          <h2 className="text-foreground mb-4 text-lg font-bold">깨달은 점</h2>
